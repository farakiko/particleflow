...
from pyg import MLPF, training_loop, make_predictions, postprocess_predictions, postprocess_predictions_clic, make_plots_cms
from pyg import save_model, load_model
from pyg import features_delphes, features_cms, target_p4, target_p4_clic
...
def train_ddp(rank, world_size, args, dataset, model, num_classes, outpath):
    ...
    # separately make 3 training and validation datasets
    # make train datasets
    train_datasets = []
    for i in range(args.num_datasets):
        train_datasets.append(torch.utils.data.Subset(dataset[i], np.arange(
            start=rank * hyper_train, stop=(rank + 1) * hyper_train)))

    valid_datasets = []
    for i in range(args.num_datasets):
        valid_datasets.append(torch.utils.data.Subset(dataset[i], np.arange(
            start=args.n_train + rank * hyper_valid, stop=args.n_train + (rank + 1) * hyper_valid)))

    file_loader_train = []
    file_loader_val = []
    # construct file loaders
    for i in range(args.num_datasets):
        file_loader_train.append(make_file_loaders(
            world_size, train_datasets[i], num_workers=args.num_workers, prefetch_factor=args.prefetch_factor))

    for i in range(args.num_datasets):
        file_loader_val.append(make_file_loaders(
            world_size, valid_datasets[i], num_workers=args.num_workers, prefetch_factor=args.prefetch_factor))

    # copy the model to the GPU with id=rank
    print(f'Copying the model on rank {rank}..')
    model = model.to(torch.float).to(rank)
    model.train()
    ddp_model = DDP(model, device_ids=[rank])

    optimizer = torch.optim.Adam(ddp_model.parameters(), lr=args.lr)

    training_loop(rank, args.data, ddp_model, file_loader_train, file_loader_val,
...
def inference_ddp(rank, world_size, args, dataset, model, num_classes, PATH):
    ...
    # hard coded values for selecting files for testing
    if args.data == "clic":
        test_dataset = torch.utils.data.Subset(dataset, np.arange(
            start = 650 + rank * hyper_test, stop = 650 + (rank + 1) * hyper_test))
    else:
        test_dataset = torch.utils.data.Subset(dataset, np.arange(
            start=rank * hyper_test, stop=(rank + 1) * hyper_test))
    ...
    make_predictions(rank, ddp_model, file_loader_test,
                     args.batch_size, num_classes, PATH)
...
def train(device, world_size, args, dataset, model, num_classes, outpath):
    ...
    # get all three datasets for training and validation
    train_datasets = []
    for i in range(len(dataset)):
        train_datasets.append(torch.utils.data.Subset(
            dataset[i], np.arange(start=0, stop=args.n_train)))

    valid_datasets = []
    for i in range(len(dataset)):
        valid_datasets.append(torch.utils.data.Subset(dataset[i], np.arange(
            start=args.n_train, stop=args.n_train + args.n_valid)))
    ...
    # training loop will take in a list of file_loaders, and will train all three together.
    training_loop(device, args.data, model, file_loader_train, file_loader_val,
                  args.batch_size, args.n_epochs, args.patience,
                  optimizer, args.alpha, args.target, num_classes, outpath)
...
def inference(device, world_size, args, dataset, model, num_classes, PATH):
    ...
    if args.data == "clic":
        test_dataset = torch.utils.data.Subset(
            dataset, np.arange(start=650, stop=650 + args.n_test))
    else:
        test_dataset = torch.utils.data.Subset(
            dataset, np.arange(start=80, stop=80 + args.n_test))
    ...
    make_predictions(device, model, file_loader_test,
                     args.batch_size, num_classes, PATH)
...
if __name__ == "__main__":
    ...
    if args.data == 'delphes':
        ...
        input_dim = 8
        num_classes = 6 # we have 9 classes for clic
        output_dim_p4 = len(target_p4_clic)
    ...
    if args.load:
        ...
        # Integrate different datasets into training
        all_datasets = []
        datasets = args.dataset.split(",")
        for i in range(args.num_datasets):
            all_datasets.append(PFGraphDataset(datasets[i], args.data))

        # only changed for the else condition
        if world_size >= 2:
            run_demo(train_ddp, world_size, args,
                     all_datasets, model, num_classes, outpath)
        else:
            train(device, world_size, args, all_datasets,
                  model, num_classes, outpath)
    ...
    if args.load and args.load_epoch != -1:
        ...
        import json
        epoch_to_load = json.load(
            open(f'{outpath}/best_epoch.json'))['best_epoch']
    ...
    if args.make_predictions:
        ...
        if world_size >= 2:
            run_demo(inference_ddp, world_size, args,
                     dataset_test, model, num_classes, PATH)
        else:
            inference(device, world_size, args,
                      dataset_test, model, num_classes, PATH)

        if args.data == "cms" or args.data == "delphes":
            postprocess_predictions(pred_path)
        elif args.data == "clic":
            postprocess_predictions_clic(pred_path)
